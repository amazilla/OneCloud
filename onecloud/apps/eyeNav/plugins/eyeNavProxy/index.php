...
// error_reporting(E_ALL); // onecloud
...
$GLOBALS['_config']            = array
                    (
                        'url_var_name'             => 'page', // onecloud
                        'flags_var_name'           => 'hl',
                        'get_form_name'            => '____pgfa',
                        'basic_auth_var_name'      => '____pbavn',
                        'max_file_size'            => -1,
                        'allow_hotlinking'         => 1, // onecloud
                        'upon_hotlink'             => 1,
                        'compress_output'          => 1 // onecloud
                    );
$GLOBALS['_flags']             = array
                    (
                        'include_form'    => 0, // onecloud
                        'remove_scripts'  => 0, // onecloud
                        'accept_cookies'  => 1,
                        'show_images'     => 1,
                        'show_referer'    => 1,
                        'rotate13'        => 0,
...
// $GLOBALS['_script_url']        = 'http' . ((isset($_ENV['HTTPS']) && $_ENV['HTTPS'] == 'on') || $_SERVER['SERVER_PORT'] == 443 ? 's' : '') . '://' . $GLOBALS['_http_host'] . ($_SERVER['SERVER_PORT'] != 80 && $_SERVER['SERVER_PORT'] != 443 ? ':' . $_SERVER['SERVER_PORT'] : '') . $_SERVER['PHP_SELF']; // onecloud
// $GLOBALS['_script_base']       = substr($GLOBALS['_script_url'], 0, strrpos($GLOBALS['_script_url'], '/')+1); // onecloud
$GLOBALS['_url']               = '';
...
if (!isset($GLOBALS['_proxify'][$GLOBALS['_content_type']]))
{
	...
	if (substr(implode('', $GLOBALS['_response_headers']['content-type']), 0, 5) == 'appli') { // onecloud
...
else
{
    ...
    for ($i = 0, $count_i = count($matches[0]); $i < $count_i; ++$i)
    {
            ...
            switch ($tag)
            {
                ...
                case 'form':
                    if (isset($attrs['action']))
                    {
                        ...
                        {
							$attrs['action'] = $GLOBALS['_script_url']; // onecloud
							$extra_html.= '<input type="hidden" name="msg" value="doOutput" />'; // onecloud
							$extra_html.= '<input type="hidden" name="checknum" value="' . $checknum . '" />'; // onecloud
                        }
        ...
        if ($rebuild)
        {
            ...
            {
                $delim = strpos($value, '"') && !strpos($value, "'") ? "'" : '"';
                $new_tag .= ' ' . $name . ($value !== false ? '=' . $delim . str_replace('&', '&amp;', $value) . $delim : ''); // onecloud
            }

            $GLOBALS['_response_body'] = str_replace($matches[0][$i], $new_tag . '>' . $extra_html, $GLOBALS['_response_body']);
        }
    }
	$GLOBALS['_response_body'] = str_replace('<body>', '<body onload=\'window.parent.sendMsg(' . $checknum . ', "SetAddress", "&lt;address&gt;' . base64_encode($GLOBALS['_url']) . '&lt;/address&gt;");\'>', $GLOBALS['_response_body']); // onecloud
